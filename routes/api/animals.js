...
router.get("/user/:user_id", (req, res) => {
  Animal.find({
    ...
    .then(animals => {
      if (!animals) {
        res.status(404).json({
          message: "There are no animals for the given user"
        });
      }
      res.json(animals);
    })
...
router.get("/:animal_id", (req, res) => {
  Animal.findById(req.params.animal_id)
    .then(animal => {
      if (!animal) {
        res.status(404).json({
          message: "There is no animal with the given ID"
        });
      }
      res.json(animal);
    })
  ...
  (req, res) => {
    ...
    Weight.find({
      ...
      .then(weights => {
        if (!weights) {
          res.status(404).json({
            message: "There is no weight data for the animal with the given ID"
          });
        }
        res.json(weights);
      })
...
router.post(
  ...
  passport.authenticate("jwt", {
    ...
    const {
      error,
      value
    } = validateAnimalInput(req.body, req.method);
      ...
      .then(animal => {
        if (animal) {
          res
            .status(409)
            .json({
              message: "An animal with those tattoos already exists"
            });
        } else {
          // Save the animal
...
router.post(
  ...
  passport.authenticate("jwt", {
    ...
    Weight.findOne({
        animal: req.params.animal_id,
        timestamp_month: req.params.timestamp_month
      }, {
        new: true
      })
      .then(weight => {
...
router.put(
  ...
  (req, res) => {
    ...
    const animalFields = parseAnimalFromBody(req);
    Animal.findOneAndUpdate(req.params.animal_id, animalFields, {
        new: true
      })
      .then(animal => {
        if (animal) {
