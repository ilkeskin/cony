...
router.get("/handle/:handle", (req, res) => {
  Profile.findOne({
    ...
    .then(profile => {
      if (!profile) {
        res.status(404).json({
          message: "There is no profile for the user with the given handle"
        });
      }
      res.json(profile);
    })
...
router.get("/user/:user_id", (req, res) => {
  Profile.findOne({
    ...
    .then(profile => {
      if (!profile) {
        res.status(404).json({
          message: "There is no profile for the user with the given ID"
        });
      }
      res.json(profile);
    })
...
router.post(
  ...
  (req, res) => {
    const {
      error,
      value
    } = validateProfileInput(req.body, req.method);
...
router.put(
  ...
  (req, res) => {
    if (req.user.id === req.params.user_id) {
      ...
      const profileFields = parseProfileFromBody(req);
      Profile.findOneAndUpdate(req.params.user_id, profileFields, {
          new: true
        })
        .then(profile => {
          if (profile) {
            ...
            res
              .status(404)
              .json({
                message: "The profile for this user does not exist"
              });
          }
        });
    } else {
